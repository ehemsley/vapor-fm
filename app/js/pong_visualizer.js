...
module.exports = class PongVisualizer extends Visualizer {
  ...
  Midlines (x, bottomY, topY, size, space) {
    const lines = []
    for (let y = bottomY, end = topY, step = size + space, asc = step > 0; asc ? y <= end : y >= end; y += step) {
  ...
  CheckBallCollision () {
    ...
    const intersects = this.ballCollisionRaycaster.intersectObjects(
      [
        this.playerPaddle,
        this.enemyPaddle,
        this.leftBound,
  ...
  PaddleUpInputPressed () {
    this.playerPaddleUp = true
  }

  PaddleDownInputPressed () {
    this.playerPaddleDown = true
  }
